...
def load_data():
    X_train = np.loadtxt("X_part1.txt")
    X_val = np.loadtxt("X_val_part1.txt")
    y_val = np.loadtxt("y_val_part1.txt")
    return X_train, X_val, y_val


def load_data_multi():
    X = np.load("X_part2.txt")
    y = np.load("y_part2.txt")
    X_val = np.load("X_val_part2.txt")
    y_val = np.load("y_val_part2.txt")
...
def print_pred_debug(
    y_p, y, user, item, maxcount=10, onlyrating=False, printfull=False
):
...
def print_existing_user(
    y_p, y, user, items, item_features, ivs, uvs, movie_dict, maxcount=10
):
