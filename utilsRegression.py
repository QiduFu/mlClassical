# This is based on the assignments completed for the Coursera specialization:
...
def plot_data(X, y, pos="y=1", neg="y=0"):
    postive = y == 1
    negative = y == 0
    plt.plot(X[postive, 0], X[postive, 1], marker="k+", c="b")
    plt.plot(X[negative, 0], X[negative, 1], marker="yo", c="r")
    ...
    if X.shape[1] <= 2:
        ...
        for i in range(len(u)):
            for j in range(len(v)):
                z[i, j] = sig(
                    np.dot(map_feature(u[i].reshape(1, -1), v[j].reshape(1, -1)), w) + b
                )
        z = z.T
        plt.contour(u, v, z, levels=[0.5], linewidths=2, colors="g")
